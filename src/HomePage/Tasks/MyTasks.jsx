...
const MyTasks = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`http://20.119.75.162:8086/apis/employees/tasksAssignedTo/${employeeId}`,{
          headers: {
  ...
  const taskTypeChange = async (event) => {
    ...
    if (event.target.value === "allTasks") {
      url = `http://20.119.75.162:8086/apis/employees/tasksAssignedTo/${employeeId}`;
    } else if (event.target.value === "overdueTasks") {
      url = `http://20.119.75.162:8086/apis/employees/OverdueTasks/PersonId/${employeeId}`;
    } else if (event.target.value === "pendingTasks") {
      url = `http://20.119.75.162:8086/apis/employees/PendingTasks/PersonId/${employeeId}`;
    } else if (event.target.value === "completedTasks") {
      url = `http://20.119.75.162:8086/apis/employees/CompletedTasks/PersonId/${employeeId}`;
    }
