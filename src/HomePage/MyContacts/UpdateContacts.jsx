...
const UpdateContacts = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
          ...
          try {
            const response = await axios.get(`http://20.119.75.162:8086/apis/employees/contacts/contacts/${contactId}`,{
                headers: {
    ...
    const handleSubmit = async(e) => {
        e.preventDefault();
        setIsAdding(true);
        await axios.put(`http://20.119.75.162:8086/apis/employees/contacts/contacts/${contactId}`, formData,{
            headers: {
              "Authorization": `Bearer ${token}`  // Add the token to the Authorization header
            }
          });
